...
export default function LearnDownload() {
  ...
  return (
    <section className="mx-auto max-w-[1500px] px-2 py-6 sm:p-8">
      ...
      {selectedFile && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4" role="dialog" aria-modal="true" aria-labelledby="download-title"><div className="relative w-full max-w-lg rounded-mission bg-white shadow-modal"><div className="rounded-t-mission bg-gradient-to-br from-dull-blue to-green p-6 text-white"><button type="button" onClick={() => setSelectedFile(null)} className="absolute right-4 top-4 p-2" aria-label="Close download form"><FaTimes /></button><h2 id="download-title" className="text-2xl font-bold">Download Chapter</h2><p>Enter your details to continue</p></div><form onSubmit={submit} className="space-y-4 p-6"><label className="block font-semibold">Full Name<input required name="name" placeholder="Enter your full name" className="mt-1 w-full rounded-xl border border-gray-300 p-3 font-normal" /></label><label className="block font-semibold">Email Address<input required type="email" name="email" placeholder="Enter your email" className="mt-1 w-full rounded-xl border border-gray-300 p-3 font-normal" /></label><p className="flex gap-2 rounded-xl bg-light-muted p-4 text-sm text-navy"><FaLock className="mt-1 shrink-0" /> Your information is safe. We only use it to track downloads and improve resources.</p><div className="flex justify-end gap-3 pt-2"><Button variant="outline" onClick={() => setSelectedFile(null)}>Cancel</Button><Button type="submit">Continue Download</Button></div></form></div></div>}
    </section>
  );
}

...
function MissionStepContent({ body, sections }: Pick<MissionStep, "body" | "sections">) {
  if (sections?.length) {
    return (
  ...
  return <p className="text-base leading-7 text-body sm:text-lg sm:leading-8">{body}</p>;
}
    ...
    >
        <div className="shrink-0 bg-dull-green p-6 text-white sm:p-8">
          ...
          <div className="flex items-center gap-4 pr-10">
            <img src={activeMission.image} alt="" className="h-16 w-16 rounded-full bg-white object-contain" />
            <div><h2 id="mission-modal-title" className="text-2xl font-bold sm:text-3xl">{activeMission.title}</h2><p className="italic text-white/80">{activeMission.intro}</p></div>
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-6 sm:p-10">
          <p className="mb-4 text-sm font-bold uppercase tracking-wider text-green">Step {step + 1} of {activeMission.steps.length}</p>
          <h3 className="mb-4 text-2xl font-bold text-dull-green">{current.title}</h3>
          <MissionStepContent body={current.body} sections={current.sections} />
        </div>

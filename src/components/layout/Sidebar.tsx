...
const sidebarActiveClass = "translate-x-1 bg-[#3e6b5a] font-semibold";

export default function Sidebar({ isOpen, onNavigate }: SidebarProps) {
    return (
        <aside
            className={`fixed bottom-0 left-0 top-header z-30 w-sidebar overflow-y-auto bg-dull-green transition-transform duration-300 lg:translate-x-0 ${
                        ...
                        return (
                            <li key={label}>
                                <NavLink
                                    ...
                                    onClick={onNavigate}
                                    className={({ isActive }) =>
                                        `flex items-center gap-3 rounded-lg px-2 py-2 text-[20px] font-normal text-white transition duration-300 hover:translate-x-1 hover:bg-[#3e6b5a] max-[576px]:text-base ${
                                            isActive ? sidebarActiveClass : ""
                                        }`
                                    }
                                >
                                    <Icon
                                        className="shrink-0 text-[23px] max-[576px]:text-[16px]"
                                        aria-hidden="true"
                                    />
                                    <span>{label}</span>
                                </NavLink>
